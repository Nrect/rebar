...
//
// ВТОРАЯ ЛОВУШКА ТОГО ЖЕ РОДА, найдена при работе над второй половиной модуля
// (сессии и адаптер): testConfig держал MaxWait в 50 мс, а потолок один на
// процесс — два слота на десятки параллельных тестов. Под -race на загруженной
// машине очередь перерастала полсекунды, и TestNeedsRehash падал «перегрузкой»
// два раза из шести. Проверено, что дефект был и до второй половины: тот же
// прогон на коммите e5f4e4f давал те же два падения из шести. Лечится
// длинным MaxWait — сам ErrBusy от него не зависит, он проверяется
// заполнением семафора (см. ниже).
//
