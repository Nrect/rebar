...
// накатывает раннер потребителя со своей таблицей версий auth_schema_version
// (ADR-0011). CheckSchema на старте сверяет схему, но не применяет её.
//
...
// Migrations() накатывает раннер потребителя (две правды о схеме, DDL-права у
// приложения и гонка реплик при выкате — не наше);
