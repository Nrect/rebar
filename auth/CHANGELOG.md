...
- `auth/password`: argon2id в формате PHC (`$argon2id$v=19$m=..,t=..,p=..$..$..`)
  ...
  заставит процесс выделить гигабайт; `Hasher` с общим на процесс потолком
...
- `auth/authtest`: `FastHasher` (настоящий argon2id на полах OWASP) и
  двойник `Strength` с заданным ответом.
