...
// closedSets — закрытые наборы пакета и их списки All*: значения уходят в метку
// метрики и в алерты потребителя.
var closedSets = map[string][]string{
	"Outcome": strs(idem.AllOutcomes),
}

// Каждая объявленная константа закрытого типа — в своём All*. Значение мимо
// списка попало бы в метку, но не в заведённые нулём ряды метрики.
...
// Значения годятся меткой: непустые, уникальные, нижний snake_case. Словарь
// исходов — ADR-0012, решение 16.
func TestClosedSetValuesAreLabelSafe(t *testing.T) {
	...
	assert.Equal(t, []string{"error", "executed", "failed", "in_flight", "not_recordable", "replayed", "reused", "too_large"},
		closedSets["Outcome"], "словарь исходов расходится с ADR-0012, решение 16")
}
