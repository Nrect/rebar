...
func catalog(t *testing.T) []string {
	t.Helper()
	names, findings := catalogFindings(idempg.Migrations())
...
func TestCatalogFindings_Fires(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "файл без номера", fsys: fstest.MapFS{"00001_a.sql": file(upDown), "idem_b.sql": file(upDown)},
			want: []string{"idem_b.sql: имя не по форме 00001_имя.sql"},
		},
		{
