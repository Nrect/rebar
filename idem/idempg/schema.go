...
// Имена, на которые ссылается код, — контракт миграций (ADR-0012, решение 15;
// CONVENTIONS §9): переименование в миграции ломало бы адаптер молча.
const (
	tableName = "idem_records"
	// uxKey — первичный ключ и арбитр ON CONFLICT в Do.
	uxKey = "ux_idem_records_key"
	// ixCreated — уборка по сроку.
	ixCreated = "ix_idem_records_created"
)

const (
	// Первая строка ошибки — что делать; расхождения перечисляются ниже неё.
	missingTableHint = "idempg.CheckSchema: таблицы idem_records нет: накатите idempg.Migrations() раннером проекта"
	mismatchHint     = "idempg.CheckSchema: схема расходится с миграциями — накатите idempg.Migrations() раннером проекта; что осталось после наката, чините своей миграцией"
)
...
var expectedColumns = map[string]string{
	"realm": typeText, "subject": typeText, "idem_key": typeText, "operation": typeText,
	"fingerprint": typeBytea, "status": typeInteger, "content_type": typeText, "location": typeText,
	"body": typeBytea, "created_at": typeTimestamptz,
}

// expectedConstraints — первичный ключ и CHECK формы.
...
var expectedIndexes = map[string]bool{uxKey: true, ixCreated: false}
...
WHERE c.oid = to_regclass('idem_records')`
