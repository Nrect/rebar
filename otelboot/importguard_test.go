...
//
// Три префикса в корне — исключение из правила «одна зависимость на причину»:
// причина одна, «бутстрап otel», но её реализация разнесена по трём модулям —
// SDK и экспортёры лежат в go.opentelemetry.io/otel, runtime-метрики в
// contrib, реестр и promhttp в client_golang. Ради этой склейки пакет и
// существует; в остальном коде потребителя её быть не должно.
