...
type Config struct {
	...
	MaxDetails int
	// MaxDetailLen — потолок длины значения подробности в байтах. Длиннее —
	// усекается до потолка и помечается Truncated, а не отвергается
	// (doc.go, п. 4), поэтому записанное значение длиннее ровно на метку.
	MaxDetailLen int
}
