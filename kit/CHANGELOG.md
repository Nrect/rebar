...
- `config` — загрузка окружения: `Loader` копит проблемы, `Err()` отдаёт их
  ...
  `Optional`, `Secret`, `OptionalSecret`, `Duration`, `Int`, `Port`, `Bool`,
  `Enum`, `Ratio`, `CSV`, `URL`, плюс `Fail` для контекстных проверок
  вызывающего. Ошибки программиста (негодное умолчание, пустой список `Enum`,
  `minLen <= 0`) — паника с текстом «X must …». Тип `Secret` редактируется в
  `fmt` (`%v`, `%+v`, `%#v`, `%q`), `slog` и `json`; значение достаёт только
  `Reveal`.
- `config.OptionalSecret(key, minLen)` — секрет, которого при этом режиме
  может не быть: отсутствующий (и пустой) ключ даёт нулевой `Secret` без
  ошибки, присутствующий проверяется тем же `minLen` той же функцией. Пока
  его не было, необязательный секрет (пароль SMTP при `SMTP_AUTH=none`)
  читали через `Optional` — то есть строкой, без типа-редактора, и он утекал
  первым же `%v`. Паника на `minLen <= 0` осталась: секрет без минимальной
  длины — не секрет, а «необязательный» относится к наличию ключа, а не к
  проверке значения.
