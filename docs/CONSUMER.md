...
отправки, при нынешнем `mail` — та же отмена (ниже). Цена правила — `outbox`
на остановке дописывает пачку хендлерами, а не возвращает её в очередь
(`releaseRest`); дубля это не даёт.
...
**Почему отменять прогон дорого — сегодня.** Два долга из
[ROADMAP, «Признанные долги»](ROADMAP.md#признанные-долги) закрываются в
блоках; до тех пор отмена посреди прогона стоит так.

- **`mail.Service.Deliver`** проверяет отмену перед каждым письмом (`waitTurn`
  в [mail/deliver.go](../mail/deliver.go)), но исход `Finish` пишет тем же
  контекстом. Отмена во время отправки оставляет строку в `sending`, и после
  `Lease` цену выбирает `mail.Config.Uncertain`: `UncertainRetry` отправит
  письмо второй раз, `UncertainPark` уведёт доставленное в `failed(uncertain)`.
  Письма той же пачки, до которых очередь не дошла, тоже остаются в `sending`:
  возврата пачки, как у `outbox`, в `mail` нет. При `UncertainRetry` они уйдут
  после `Lease` без дубля, при `UncertainPark` — в `failed(uncertain)`, хотя не
  отправлялись вовсе.
- **`objectstore.Collector.Run`** сворачивает сбой хранилища в `ErrUnavailable`
  без причины: на `s3` отменённый прогон пишется как «хранилище недоступно»,
  `result="error"` без сбоя.

Что проект делает, пока долги открыты:

1. контекст задач сигналом не отменяется (выше);
2. бюджет `Stop` покрывает обычную пачку `mail`, а `BatchSize` подобран под
   бюджет;
3. `Uncertain` выбирается с учётом остановки: процесс, убитый после
   исчерпанного бюджета, оставляет в `sending` до `BatchSize` писем, и при
   `UncertainPark` каждое разбирает человек;
4. единичный `result="error"` у сборщика объектов на остановке — не инцидент:
   алерт по задаче строится на давность последнего успеха, как «крон умер» в
...
|---|---|---|
| `mail.Service.Deliver` | пачка в `sending` до конца `Lease` | следующий `Claim`, дальше — `Config.Uncertain` |
| `outbox.Worker.Drain` | пачка в `processing` под арендой | после `Lease` хендлер повторяется с `Reclaimed`, поэтому он обязан быть идемпотентным |
