...
приложению роль без DDL. Локер — новый на каждый провайдер: у
`lock.NewPostgresSessionLocker` счётчик неудачных попыток взять блокировку живёт
в самом локере и между вызовами не сбрасывается, и под конкуренцией реплик
общий локер делил бы одни попытки на все блоки (образец —
[`examples/monolith/shoppg/migrate.go`](../examples/monolith/shoppg/migrate.go)).
...
**Всё время процесса — UTC, и от пояса машины процесс не зависит.** Закон и
причины — [CORRECTNESS §8](CORRECTNESS.md#8-время--utc-и-параметр); здесь — что
делает проект вокруг блоков. Блоки своё уже держат: часы в UTC, `timestamptz`,
страж `scripts/timeguard.sh` и тесты под чужим поясом (CONVENTIONS §11).
...
2. **Схема проекта — `timestamptz`, время — параметром.** `now()` и
   ...
   pgx отдаёт `timestamptz` в поясе процесса.
3. **Лог пишет время в UTC сам.** `slog` берёт время записи из `time.Now()`, то
   есть в поясе процесса: контейнер с `TZ=Europe/Moscow` пишет `+03:00`, и
   записи двух сервисов не сходятся по времени. Обработчик переводит ключ
   времени в UTC:
...
4. **Бизнес-пояс — из окружения, на старте.** «Сутки по Москве», отчёт за месяц,
   рассылка в 10:00 считаются в `*time.Location` из переменной (например
   `BUSINESS_TZ=Europe/Moscow`), загруженном `time.LoadLocation` при сборке
   конфига: неизвестный пояс — отказ старта, а не первого отчёта. В `main` —
   `import _ "time/tzdata"`: в образах `scratch` и distroless базы поясов нет, и
   без этого `LoadLocation` падает только в проде. Считается в поясе, хранится в
   UTC. Планировщик тулкита знает интервалы, а не часы суток: задача «в 03:00»
   сверяет текущий момент в бизнес-поясе внутри прогона.
5. **Контейнеры — `TZ=UTC` у приложения и у Postgres.** Это второй рубеж, первые
   — пункты 1–3. Официальный образ Postgres и так в UTC, но явная строка
   переживает смену образа и чужой `postgresql.conf`.
6. **Наружу — RFC 3339 с `Z`.** `time.Time` в UTC сам ложится в JSON с `Z`;
   момент без пояса во входящем JSON не разбирается, а чужой зажимается в
   допустимое окно (CORRECTNESS §8).
7. **Тесты проекта — под поясом не UTC** (`TZ=America/St_Johns go test ./...`),
