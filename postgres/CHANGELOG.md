...
### Added
- `pgtest`: помощники каталога миграций блока (ADR-0011, решение 8), чтобы
  семь адаптеров не писали по копии одного цикла. `ApplyUp(tb, pool, fsys)` —
  секции Up всех файлов по возрастанию номера, `ApplyDown` — Down по убыванию;
  файл уходит одним запросом. Это накат на свежую схему теста, а не раннер:
  таблицы версий нет, зависимости на goose по-прежнему нет.
- `pgtest.CheckMigrations(tb, fsys)` — стражи каталога (решение 7, пункты
  1–3): номера `00001_…` в пять знаков идут подряд с единицы без повторов,
  файла без номера нет, у каждого файла ровно по секции Up и Down, и Up
  раньше. Каждая находка — своя ошибка теста с именем файла; база не нужна.
  Каталог с находками `ApplyUp` и `ApplyDown` не применяют вовсе.
- `pgtest.SchemaObjects(tb, pool)` — таблицы, индексы, последовательности,
  представления, функции и типы, оставшиеся в текущей схеме: пусто после
  `ApplyDown` значит, что откат снял всё. Функции считаются наравне с
  таблицами — `DROP TABLE` функцию триггера не снимает, а идемпотентный Up её
  повторным накатом уже не выдаст. Пул без текущей схемы — падение теста, а не
  «пусто».
- Образец каталога — `pgtest/testdata/migrations/good`: идемпотентные Up и
  Down (уточнение 1), триггер через `DROP` + `CREATE` + `ENABLE ALWAYS`.
