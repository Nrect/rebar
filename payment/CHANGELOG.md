...
- **Документация: `HandleWebhook` отвечает провайдеру классом, `IntentByKey`
  — не проверка повтора.** У `HandleWebhook` строка у спускового крючка:
  правило `Translate` потребителя, совпавшее с ошибкой хука под
  `ErrUnavailable`, превратило бы 503 в 4xx. У `IntentByKey` снята фраза,
  подсказывавшая пробу до `Start`: найденное намерение, отданное успехом,
  обходит сверку отпечатка и отказ закрытой попытки — пример-монолит так и
  отвечал 200 на повтор ключа после отказа провайдера. Код не менялся.
- **`paymenttest.MemStore` замечает отменённый контекст: каждый метод порта
  отвечает `payment.ErrUnavailable` с `context.Canceled` в цепочке — как
  `paymentpg`, где отмена не доезжает до базы.** Раньше двойник контекст не
  смотрел: тест потребителя «вебхук отменили — деньги не учтены» был зелёным
  на двойнике и красным в проде. Порядок проверок тот же, что у адаптера: то,
  что `paymentpg` решает ДО транзакции — форма возврата (`ApplyRefund`) и
  размер пачки (`StalePending`, `Drift`), — отмена не перебивает. Ломающее для
  теста потребителя, который передавал в двойник отменённый контекст и ждал
  успеха.
- Сценарий контрактного набора «отменённый контекст — ErrUnavailable»
  (`paymenttest.RunStoreSuite`): расхождение двойника и `paymentpg` на отмене
  краснеет теперь в наборе, а не у потребителя.
