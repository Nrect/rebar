...
//     вопросе, есть ли ещё деньги. Отсюда обязанность АДАПТЕРА: любое
//     терминальное «денег не будет» на холде (срок холда истёк, провайдер
//     отменил, списание отклонено окончательно) отображается в EventCanceled, а
//     не в EventFailed. Словарь провайдера переводит адаптер, домен видит одну
//     клетку. Держит TestReconcile_HoldIsNeverExpiredByTTL.
//
