...
//
// Отвечать провайдеру классом, без продуктового перевода: правило потребителя,
// совпавшее с ошибкой хука под ErrUnavailable, превратило бы 503 в 4xx.
//
...
//
// НЕУВЕРЕННОСТЬ ПАДАЕТ В СТОРОНУ НАШЕГО СБОЯ. Повторов код ответа не
// останавливает: проверенные провайдеры повторяют всё, кроме подтверждения
// (ADR-0012, «Снаружи»). Решает он, чей это сбой. Ошибочный 503 стоит лишней
// тревоги на заведомо плохом теле; ошибочный 400 выдаёт наш сбой за негодное
// уведомление — 4xx идёт в лог на Debug и мимо алертов на 5xx, и когда повторы
// кончатся, деньги останутся не зачисленными без сигнала. Поэтому 400 —
