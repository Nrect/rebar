...
// (TestMoneySub_*).
//
// Повторить: make mutants MODULE=payment \
//     MUTANTS_EXCLUDE="-E '^paymenttest/' -E '^prorate/' -E '^paymentpg/'"
// (коэффициент таймаута 20 и GOWORK=off — в самой цели Makefile; на меньшем
// коэффициенте прогон врёт зелёным.)
//
// АДАПТЕР ИСКЛЮЧЁН, и это не экономия времени: его держат интеграционные
// тесты (CONVENTIONS §5). Оставленный в прогоне paymentpg добавляет две
// минуты, пятерых выживших и восемь просрочек — шум, в котором тонет
// единственный выживший ядра. DOCKER_HOST при этом всё равно обязателен:
// покрытие gremlins снимает всем набором модуля, включая интеграционные
// тесты, и без него прогон падает на failed to gather coverage.
