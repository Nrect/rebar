...
// точен, а не интерполирован. Умолчание SDK рассчитано на миллисекунды. Страж —
// TestObserver_DurationBuckets.
//
...
//     заголовки и текст ошибки в порт не приходят вовсе. Страж —
//     TestObserver_EveryPointIsFromClosedSets.
//  2. РЯДЫ РОЖДАЮТСЯ НУЛЁМ ПРИ СБОРКЕ СЕРВИСА: inbox.NewService зовёт Watch для
//     каждого источника Config до первого Receive — тот же набор, что сверен с
//     хранилищем, и забыть источник негде. Ряд, появившийся сразу единицей,
//     increase() не видит, а у алерта conflict порог 1. Страж —
//     TestObserver_WatchStartsEverySeriesAtZero.
//  3. МЕТКА — ИСХОД, КОТОРЫЙ ОТДАЛ RECEIVE (PATTERNS §8), по каждой доставке, а
//     не по итогу: два исхода, поменявшиеся метками, итог не меняют. Страж —
//     TestObserver_MatchesService.
//  4. ГИСТОГРАММА НУЛЁМ НЕ ЗАВОДИТСЯ: записанный ноль — доставка, которой не
//     было, в счёте и в p99. Страж — TestObserver_WatchStartsEverySeriesAtZero.
//  5. ОТМЕНЁННЫЙ КОНТЕКСТ СЧЁТ НЕ ТЕРЯЕТ: доставка, которую оборвал
//     отправитель, — тоже доставка. Страж —
//     TestObserver_CountsUnderCanceledContext.
//  6. FAIL CLOSED НА СБОРКЕ: nil-метр — паника; ошибка создания инструмента
//     возвращается — метрик у потребителя может не быть, а приём нужен.
//     Стражи — TestNewObserver_PanicsOnNilMeter,
//     TestNewObserver_ReturnsInstrumentError.
//
