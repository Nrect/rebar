...
//
// ЛОВУШКА: baseline gremlins без этого файла не детерминирован (15/13/15
// выживших на трёх прогонах). Флапали мутанты границ в deliver.go:66,
// message.go:22 и в копии backoff, ушедшей в kit/retry, — из-за тестов на
// реальных часах (таймер MinSendGap, SendTimeout, гонка двух сервисов) и
// per-mutant timeout. С этим файлом набор выживших стабилен, все они ниже.
//
// ЧЕТВЁРТЫЙ ВЫЖИВШИЙ ЖИЛ ЗДЕСЬ НЕЗАМЕЧЕННЫМ: service.go:83, арифметика
// len(Text)+len(HTML) внутри ТЕКСТА ошибки о превышении потолка. Условие
// строкой выше сторожили тесты, а число в сообщении — никто, и «стабильно
// три» в прежней редакции было утверждением о трёх при четырёх. Убит
// TestPrepare_TooLargeBodyNamesRealSize: Text и HTML разной длины, сообщение
// обязано назвать их СУММУ. Урок общий: выражение в аргументах fmt.Errorf —
// такой же код, как в условии, и мутанты в нём выживают ровно потому, что на
// текст ошибки утверждений обычно не пишут.
//
// BACKOFF ЖИВЁТ В kit/retry: mail.Backoff — псевдоним, мутанты экспоненты и
// разбор эквивалентных — в kit/retry/mutants_internal_test.go. Тесты backoff
// ниже остались и гоняют kit через псевдоним: доставка держится на свойстве,
// а не на копии.
//
