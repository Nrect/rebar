...
- **Исход `FinishReleased` — возврат строки, до отправки которой отменённый
  прогон не дошёл**: `pending` немедленно, аренда снята, попытка, взятая
  `Claim`, возвращена (не ниже нуля); `next_attempt_at`, `last_error` и
  `transport` не меняются. `mailpg` пишет его одним `UPDATE`, схема не
  меняется; двойник — так же. Держит сценарий набора «released возвращает
  попытку и место в очереди» — по `mailtest.MemStore` и по `mailpg.Store`.
  Ломающее для своей реализации `mail.Store`: неизвестный ей исход — отказ
  `Finish`, и остановленный прогон отдаст `ErrUnavailable`, оставив остаток
...
### Fixed
- **Отмена `Deliver` больше не стоит дубля, ложного `failed` и сожжённых
  попыток: исход отправленного письма пишется мимо отмены, а невзятый остаток
  пачки возвращается в очередь.** Было: `Claim` переводит в `sending` всю
  пачку, а отмена рвала и запись исхода (`Finish` шёл по отменённому
  контексту), и остаток никто не возвращал. Всё взятое ждало `Lease` и
  приходило с `Reclaimed`: при `UncertainRetry` отправленное письмо уезжало
  второй раз, а невзятые теряли попытку; при `UncertainPark` в `failed` уходила
  вся пачка — и доставленное письмо, и ни разу не отправленные. Совет «давайте
  остановку паузой между письмами — это бесплатно» (прежний пункт этого
  раздела и `README`) был неверен при `UncertainPark`.

  Стало: исход и возврат (`FinishReleased`, по образцу `outbox`) идут по
  `context.WithoutCancel` со сроком `Config.SendTimeout` — на исход строки и
  отдельно на возврат остатка. Новое поле не заведено: это потолок шага
  строки, который потребитель уже задал, и повисшая база держит остановку не
  дольше. Срок действует и без отмены: запись исхода дольше `SendTimeout` —
  сбой `Finish`, `ErrUnavailable`. `processed` считает и возвращённые строки,
  как `outbox.Drain`.

  | Отмена пришла | Строка |
  |---|---|
  | между письмами | возвращена: `pending`, попытка цела, уходит следующим прогоном |
  | посреди отправки, а провайдер ответил | исход записан |
  | до отправки, посреди проверки стоп-листа | возвращена: ответу, оборванному отменой, не верим |
  | посреди отправки и оборвала её | исход НЕ записан и строка не возвращена: письмо могло уйти. Ждёт `Lease` и `Config.Uncertain`, как при падении процесса — записанный повтор или возврат обошёл бы `UncertainPark`, а исчерпанная попытка сожгла бы письмо |
  | строка взята с `Reclaimed` | не возвращается: возврат стёр бы знание, что прошлая попытка не завершилась. Ждёт `Lease`, решает `Config.Uncertain` |

  `Deliver` с `context.Canceled` без `ErrUnavailable` в цепочке — остановленный
  прогон, а не сбой хранилища; `README`, «Фоновые задания», логирует только
  `ErrUnavailable`. Держат `TestDeliver_CancelDuringSendRecordsOutcome` (в
  ядре и в `mailpg` на живой базе), `CancelBetweenRowsReleasesRest`,
  `CancelBeforeSendReleasesRow`, `SendCutByCancelIsLeftToUncertainPolicy`,
  `ReleaseKeepsReclaimedRowUnderLease`, `HungFinishDoesNotOutliveBudget` и
  `HungReleaseDoesNotOutliveBudget`.
