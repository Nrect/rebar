...
- **Документация: отмена ПОСРЕДИ отправки стоит дубля или ложного `failed`.**
  Письмо уже ушло, а `Finish` идёт по тому же отменённому контексту и до базы
  не доезжает: `Deliver` отдаёт `ErrUnavailable` с `context.Canceled` в цепочке
  и `processed = 0`, строка остаётся в `sending`. После истечения `Lease` её
  заберёт следующий прогон с `Reclaimed`, и дальше решает `Config.Uncertain`:
  `UncertainRetry` — письмо уедет второй раз, `UncertainPark` — строка уйдёт в
  `failed` с `FailUncertain`, хотя письмо доставлено. Окно то же, что у упавшего
  процесса, ради которого заведены аренда и `Reclaimed`; отмена лишь достижима
  дешевле. Так было в проде всегда — зелёным обратное утверждение держал
  двойник, не смотревший контекст (`TestDeliver_ContextCancelStopsBatch`).
  Отмена МЕЖДУ строками не стоит ничего: её ловит `waitTurn` до отправки,
  поэтому остановку лучше давать паузой между письмами, а не в середине.
  Код не менялся.
