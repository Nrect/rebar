...
- **Документация: отмена ПОСРЕДИ отправки стоит дубля.** Письмо уже ушло, а
  `Finish` идёт по тому же отменённому контексту и до базы не доезжает: строка
  остаётся в `sending` и после истечения аренды уедет второй раз, `Deliver`
  отдаёт `ErrUnavailable` с `context.Canceled` в цепочке и `processed = 0`.
  Так было в проде всегда; зелёным обратное утверждение держал двойник, не
  смотревший контекст (`TestDeliver_ContextCancelStopsBatch`). Отмена МЕЖДУ
  строками дубля не стоит — её ловит `waitTurn` до отправки. Код не менялся.
