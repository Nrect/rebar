...
// добавляется сюда тем же коммитом, что и каталог. Ядро — только stdlib:
// наблюдаемость уходит через порт Observer, otel живёт в schedulerotel
// (CONVENTIONS §6).
var allowedByDir = map[string][]string{
	".":             {},
	"schedulertest": {},
	"schedulerotel": {"go.opentelemetry.io/otel/metric", "go.opentelemetry.io/otel/attribute"},
}
