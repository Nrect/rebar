...
type Config struct {
	...
	//
	// ЗДЕСЬ ВЫБИРАЕТСЯ ORIGIN, В КОТОРОМ ИСПОЛНИТСЯ СОДЕРЖИМОЕ. Пока ссылка
	// ведёт на бакет, файл живёт в чужом origin и нам не опасен; поставив сюда
	// СВОЙ домен, потребитель переносит его в свой — а PDF умеет нести
	// JavaScript, и браузер рисует его встроенным просмотрщиком. Свой домен —
	// значит Content-Disposition: attachment на отдаче (doc.go ядра, п. 11).
	// Заголовков пакет не ставит: отдаёт файлы не он.
	PublicBase string
