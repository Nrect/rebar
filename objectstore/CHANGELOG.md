...
- **Класс ошибки у sentinel ([ADR-0007](../docs/adr/0007-error-kind.md)).**
  ...
  | `ErrNotFound` | нет, `//errs:nokind` | в модуле её отдаёт только `s3` на ответ 404, а у `Put`, `Delete` и `List` это нет бакета — дефект конфигурации, то есть 500. Правило примера-монолита на неё (404 `file-not-found`) снято |
...
- **API двойников меняется ломающе: настройка `objectstoretest.MemStore` и
  ...
  двойника, изнутри `Put`. `nil` вместо часов паникует, как у ядра (ниже).
...
- **Сбой хранилища `objectstoretest.MemStore` приходит в
  ...
  Сбой `MemOwned` приходит голым, как раньше: источник владения пишет
  потребитель.
- **`objectstoretest.MemStore.SetClock(nil)` паникует, а не откладывает отказ
  до первого `Put`.** Настройка падает громко и на старте
  ([CONVENTIONS §2](../CONVENTIONS.md)), как `objectstore.Collector.SetClock` и
  `objectstore/s3.Store.SetClock`; это последняя из тринадцати реализаций
  `SetClock`, принимавшая нулевые часы молча. Ломающее: **`ErrDoubleBroken`
  удалена** — её отдавал единственный путь, тот самый отложенный отказ.
  Замена: паника `objectstoretest.MemStore.SetClock: now must not be nil` в
  месте настройки.
- **`objectstoretest.MemStore` замечает отменённый контекст, как `s3`: `Put`,
  `Delete` и `List` отвечают `objectstore.ErrUnavailable` без причины в
  цепочке и хранилище не меняют.** Раньше двойник контекст не смотрел, как
  `fs`, и равняться на `s3` не мог: `Collector.Run` терял причину отмены
  (ниже, «Fixed»), и падающий двойник сделал бы контракт отмены сборщика
  недостижимым. Ядро починено — двойник равняется на адаптер прода, а не
  стенда; `fs` контекст по-прежнему не читает. Проверки, которые `s3` делает
  до запроса, отмена не перебивает: негодный ключ остаётся `ErrBadKey`,
  непозитивный лимит `List` — пустой страницей; `Presign` отмены не видит.
  Держат `TestMemStore_CancelledContextIsUnavailableLikeS3` и парный ему
  `TestS3_CancelledContextIsUnavailableWithoutCause`. Ломающее для теста
  потребителя, который передавал в двойник отменённый контекст и ждал успеха
  или `context.Canceled` в цепочке — у `s3` её нет.
