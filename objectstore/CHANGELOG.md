...
- **`objectstoretest.MemStore.SetClock(nil)` паникует, а не откладывает отказ
  ...
  месте настройки.
- **Отменённый контекст `MemStore` по-прежнему не смотрит — но теперь это
  записанное решение, а не умолчание.** Довод у самого типа: адаптеры модуля
  расходятся (`s3` на отмене падает транспортом в `ErrUnavailable` без причины
  в цепочке, `fs` контекст не читает вовсе), и совпасть с обоими нельзя.
  Выбран `fs`, потому что равнение на `s3` упирается в ядро: `Collector.Run`
  сворачивает отказ `List` в `ErrUnavailable` без причины, и его собственный
  контракт «отмена останавливает прогон с `context.Canceled`» держится ровно
  на том, что `List` отмену пропускает, — падающий двойник сделал бы этот
  контракт недостижимым ни для одной реализации. Расхождение с `s3` остаётся
  долгом и чинится в ядре. Решение сторожит
