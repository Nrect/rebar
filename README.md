...
| `auth/` | `github.com/nrect/rebar/auth` | вход и учётные данные: argon2id с общим на процесс потолком одновременности, политика силы пароля, одноразовые токены под секретом реалма; `pwzxcvbn` — оценка силы адаптером | реализованы типы, пароли и токены; сессии, `authpg` и `authhttp` — следующей задачей |
