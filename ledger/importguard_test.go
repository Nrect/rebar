...
//
// Каталоги адаптеров описаны ЗАРАНЕЕ, до того как появились: их пишут
...
var allowedByDir = map[string][]string{
	// kit — класс у sentinel (errs.Kinded, ADR-0007) и номер ключа подписи
	// (secrets.KeyID); зависимость на kit разрешена отовсюду (ADR-0005).
	".":          {"github.com/google/uuid", "github.com/nrect/rebar/kit"},
	"ledgertest": {"github.com/google/uuid", "github.com/nrect/rebar/kit"},
	// postgres — граница ошибки Postgres, разрешённая адаптерам хранилища
	// (ADR-0005); ядру она запрещена.
	"ledgerpg":   {"github.com/google/uuid", "github.com/jackc/pgx/v5", "github.com/nrect/rebar/postgres", "github.com/nrect/rebar/kit"},
	"ledgerotel": {"go.opentelemetry.io/otel/metric", "go.opentelemetry.io/otel/attribute"}, // без uuid: счёт в метке — ПДн
}
