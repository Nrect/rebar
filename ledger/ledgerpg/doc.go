...
//
// Восстановление через pg_restore --disable-triggers возвращает триггеры режимом
// ENABLE, а не ALWAYS: после него накатите миграции заново — CheckSchema иначе
// назовёт каждый триггер. Данные, правленые в окне восстановления, видит только
// сверка (ledger.Reconciler).
//
// Безопасность:
//
...
//  10. ПРАВКА В ОКНЕ ВОССТАНОВЛЕНИЯ НЕ ПРОХОДИТ МОЛЧА (условие выпуска 8): с
//     отключёнными триггерами база не держит ничего, и сверка видит правку
//     суммы и остатка, удалённую запись, чужую подпись, потерянную строку счёта
//     и откаченную голову; обход счетов идёт и по журналу. Страж —
//     TestDrill_RestoreWithTriggersDisabled.
//
