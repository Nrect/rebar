...
#
# Свои модули тоже здесь. Разъезд межмодульного пина гейт иначе не ловит: пока
# у модулей нет тегов, они ссылаются друг на друга псевдоверсиями, а
# `go get @latest` отдаёт устаревшую (прокси кэширует список версий). Так один
# модуль полдня жил на коммите до починки разбора миграций — молча и без
# единого красного теста.
prefixes="github.com/nrect/rebar
