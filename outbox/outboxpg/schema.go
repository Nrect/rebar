...
var expectedColumns = map[string]string{
	...
	"payload":        typeJSONB,
	"headers":        typeJSONB,
	"aggregate_type": typeText,
	"aggregate_id":   typeText,
	"schema_version": typeInteger,
	"dedup_key":      typeText,
	"fingerprint":    "bytea",
	"status":         typeText,
	"attempts":       typeInteger,
	"available_at":   typeTimestamptz,
	"not_after":      typeTimestamptz,
	"occurred_at":    typeTimestamptz,
	"claim_token":    typeUUID,
	"locked_until":   typeTimestamptz,
	"last_error":     typeText,
	"fail_reason":    typeText,
	"created_at":     typeTimestamptz,
	"updated_at":     typeTimestamptz,
	"done_at":        typeTimestamptz,
}
...
var expectedIndexes = map[string]bool{
	"ux_outbox_messages_dedup":     true, // арбитр ON CONFLICT в Enqueue
	"ix_outbox_messages_due":       false,
	"ix_outbox_messages_terminal":  false,
	"ix_outbox_messages_failed":    false,
	"ix_outbox_messages_aggregate": false,
}
...
WHERE c.oid = to_regclass('outbox_messages')`
	...
	for _, name := range slices.Sorted(maps.Keys(expectedColumns)) {
		...
		case got != expectedColumns[name]:
			problems = append(problems,
				fmt.Sprintf("колонка %s: тип %s, ожидается %s", name, got, expectedColumns[name]))
		}
	}
	return problems, nil
}
