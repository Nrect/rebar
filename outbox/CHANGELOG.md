...
- **Отмена `Drain` после ответа хендлера больше не стоит повтора его эффекта:
  ...
  вызывающего, у задачи планировщика обычно без срока). При
  повисшей базе запись исхода и возврат остатка держат остановку до двух
  `HandlerTimeout` сверх работы хендлера.
