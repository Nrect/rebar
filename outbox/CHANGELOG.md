...
- `outboxtest.RunStoreSuite`: сценарий «только отложенные строки — возраст
  ноль, а не отрицательный». Закрепляет в контракте то, что двойник и
  `outboxpg` уже делали: `OldestDueAge` считается только по готовым строкам.
  Прежний сценарий возраста этот фильтр не различал — самой старой строкой там
  была готовая. Контракт записан у `outbox.Store.Stats`.
...
### Fixed
- **Отмена `Drain` после ответа хендлера больше не стоит повтора его эффекта:
  исход пишется мимо отмены прогона.** Было: `Finish` шёл по контексту
  вызывающего, и отмена, пришедшая, пока хендлер работал, не пускала запись
  исхода до хранилища. Строка ждала `Lease` и приходила снова с `Reclaimed` —
  хендлер исполнялся второй раз на каждой штатной остановке, — остаток пачки
  тоже ждал аренды с потраченной попыткой, а `Drain` отдавал `ErrUnavailable`,
  как при сбое базы.

  Стало: исход идёт по `context.WithoutCancel` со сроком
  `Config.HandlerTimeout` — тем же, что с `v0.1.0` ограничивает возврат остатка
  (`FinishReleased`); после оборванного хендлера остаток тоже возвращается. Новое
  поле не заведено: `HandlerTimeout` — уже заданный потребителем потолок шага
  строки, обязательное новое поле отказало бы на старте каждому потребителю, а
  необязательное спорило бы с правилом `Config` «нулевое значение — отказ на
  старте». Срок действует и без отмены: запись исхода дольше `HandlerTimeout` —
  сбой `Finish`, `ErrUnavailable` (раньше — сколько позволял контекст
  вызывающего, у задачи планировщика обычно без срока). При повисшей базе
  запись исхода и возврат остатка держат остановку до двух `HandlerTimeout`
  сверх работы хендлера.

  | Отмена пришла | Строка |
  |---|---|
  | до старта хендлера | возвращена: `pending`, попытка цела (как раньше) |
  | пока хендлер работал, а он ответил сам: `nil`, `ErrSkip`, `Permanent` или `Throttled` | исход записан, после `Lease` хендлер не повторяется; остаток пачки возвращён |
  | и оборвала хендлер: ошибка без класса или паника при отменённом `ctx` | исход НЕ записан и строка не возвращена: эффект мог случиться. Ждёт `Lease` и приходит с `Reclaimed`, как при падении процесса: записанный повтор стёр бы `Reclaimed`, а исчерпанная попытка увела бы строку в `failed(exhausted)`. Остаток пачки возвращён |
  | строка взята с `Reclaimed` | не возвращается ни до хендлера, ни в остатке: ждёт `Lease` и приходит с `Reclaimed` снова (пункт ниже) |

  Класс ошибки хендлер называет осознанно, поэтому `Permanent` и `Throttled`
  при отменённом `ctx` — ответ, а не обрыв; ошибка без класса от вызванной
  отменой неотличима. `Drain` с `context.Canceled` без `ErrUnavailable` в
  цепочке — остановленный прогон, а не сбой хранилища; дописав последнюю
  строку пачки, `Drain` отдаёт `nil`. Заметно коду, который ждал
  `ErrUnavailable` от отмены посреди хендлера или повтора хендлера после
  остановки. Держат `TestDrain_CancelAfterHandlerAnswerRecordsOutcome` и
  `TestDrain_HandlerCutByCancelIsLeftForLease` (в ядре и в `outboxpg` на живой
  базе), `ClassifiedAnswerUnderCancelIsRecorded`,
  `HungFinishDoesNotOutliveBudget` и `HungReleaseDoesNotOutliveBudget`.
- **Остановка пачки больше не стирает `Reclaimed`: строка, взятая из
  `processing` с истёкшей арендой, при остановке не возвращается в очередь, а
  ждёт аренды.** Было: возврат остатка (`FinishReleased`) брал всю невзятую
  часть пачки, и строка, чья прошлая попытка не досказала исход, уходила в
  `pending`; следующий `Claim` отдавал её хендлеру уже без `Reclaimed`, и
  обещание `doc.go`, п. 2, нарушалось ровно на штатной остановке после сбоя.
  Стало, как у `mail`: такая строка остаётся под арендой и приходит с
  `Reclaimed`. Цена — ожидание `Lease` у неё одной и попытка, взятая `Claim`,
  которая не возвращается. Держит `TestDrain_ReleaseKeepsReclaimedRowUnderLease`.
