...
//
// МОМЕНТЫ — КАК В timestamptz: реализация отдаёт их в UTC с точностью до
// микросекунд и с той же точностью сравнивает параметры (аренда в Claim,
// Purge). Хранилище, отдающее наносекунды и чужую зону, зеленит у потребителя
// сравнение меток, которое на базе красное (outboxtest.RunStoreSuite).
